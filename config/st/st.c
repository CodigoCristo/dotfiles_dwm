...
#define SELLINE(y)      ((y) < sel.scr ? term.hist[((y) + term.histi - \
            sel.scr + HISTSIZE + 1) % HISTSIZE] : \
            term.line[(y) - sel.scr])
...
typedef struct {
	...
	int alt;
        int scr;   /* term.scr en el momento de crear la selección */
} Selection;
...
sellinelen(int y)
{
	...
	if (SELLINE(y)[i - 1].mode & ATTR_WRAP)
		return i;

	while (i > 0 && SELLINE(y)[i - 1].u == ' ')
		--i;
...
getsel(void)
{
	...
	for (y = sel.nb.y; y <= sel.ne.y; y++) {
		if ((linelen = sellinelen(y)) == 0) {
			*ptr++ = '\n';
			continue;
		}

		if (sel.type == SEL_RECTANGULAR) {
			gp = &SELLINE(y)[sel.nb.x];
			lastx = sel.ne.x;
		} else {
			gp = &SELLINE(y)[sel.nb.y == y ? sel.nb.x : 0];
			lastx = (sel.ne.y == y) ? sel.ne.x : term.col-1;
		}
		last = &SELLINE(y)[MIN(lastx, linelen-1)];
...
kscrolldown(const Arg* a)
{
	...
	if (term.scr > 0) {
		...
		if (sel.ob.x != -1 && sel.alt == IS_SET(MODE_ALTSCREEN))
			selextend(sel.oe.x, sel.oe.y + n, sel.type, 0);
		tfulldirt();
	}
}
