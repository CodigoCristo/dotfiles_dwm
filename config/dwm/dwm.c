...
drawbar(Monitor *m)
{
	...
	unsigned int bw = m->ww; /* ancho real disponible en la barra */
	Client *c;
	...
	/* --- letra d minúscula --- */
	/* asta: vertical completa a la derecha (como una "l") */
	drw_rect(drw, x + lw - stroke, pad, stroke, lh, 1, 0);
	/* panza: solo mitad inferior, abierta arriba-izquierda */
	drw_rect(drw, x, pad + lh / 2, stroke, lh / 2, 1, 0);           /* vertical izquierda (mitad) */
	drw_rect(drw, x, pad + lh - stroke, lw, stroke, 1, 0);          /* horizontal inferior */
	drw_rect(drw, x, pad + lh / 2, lw, stroke, 1, 0);               /* horizontal media (cierra panza arriba) */
	x += lw + gap;

	/* --- letra w --- */
	drw_rect(drw, x, pad, stroke, lh, 1, 0);
	drw_rect(drw, x, pad + lh - stroke, lw, stroke, 1, 0);
	drw_rect(drw, x + lw / 2 - stroke / 2, pad + lh / 2, stroke, lh / 2, 1, 0);
	drw_rect(drw, x + lw - stroke, pad, stroke, lh, 1, 0);
	x += lw + gap;

	/* --- letra m --- */
	drw_rect(drw, x, pad, stroke, lh, 1, 0);
	drw_rect(drw, x, pad, lw, stroke, 1, 0);
	drw_rect(drw, x + lw / 2 - stroke / 2, pad, stroke, lh, 1, 0);
	drw_rect(drw, x + lw - stroke, pad, stroke, lh, 1, 0);

	/* comienza a dibujar los tags después del logo */
	x = dwmlogowdth;
...
tile(Monitor *m)
{
	...
	if (smartgaps == n)
		oe = 0; /* gap externo desactivado si solo hay 1 ventana */
